...
func NewUnicorn(data UnicornData) *Unicorn {
	...
	tipPos := u.HornOnset.Center.Shifted(Point3d{-10, 0, 0})
	...
	u.TailStart = NewBallP(u.Butt.Center.Shifted(Point3d{10, -10, 0}), data.TailStartSize, data.Color("Hair", 80))
...
func (u *Unicorn) makeEyes(data UnicornData) {
	...
	u.BrowLeftInner = NewBallP(u.EyeLeft.Center.Shifted(Point3d{0, -10, data.BrowLength}), data.BrowSize, data.Color("Hair", 50))
	u.BrowLeftInner.SetGap(5+moodDelta, *u.EyeLeft)
	u.BrowLeftMiddle = NewBallP(u.EyeLeft.Center.Shifted(Point3d{0, -10, 0}), data.BrowSize, data.Color("Hair", 70))
	u.BrowLeftMiddle.SetGap(5+data.BrowLength, *u.EyeLeft)
	u.BrowLeftOuter = NewBallP(u.EyeLeft.Center.Shifted(Point3d{0, -10, -data.BrowLength}), data.BrowSize, data.Color("Hair", 60))
	u.BrowLeftOuter.SetGap(5-moodDelta, *u.EyeLeft)

	u.BrowRightInner = NewBallP(u.EyeRight.Center.Shifted(Point3d{0, -10, -data.BrowLength}), data.BrowSize, data.Color("Hair", 50))
	u.BrowRightInner.SetGap(5+moodDelta, *u.EyeRight)
	u.BrowRightMiddle = NewBallP(u.EyeRight.Center.Shifted(Point3d{0, -10, 0}), data.BrowSize, data.Color("Hair", 70))
	u.BrowRightMiddle.SetGap(5+data.BrowLength, *u.EyeRight)
	u.BrowRightOuter = NewBallP(u.EyeRight.Center.Shifted(Point3d{0, -10, data.BrowLength}), data.BrowSize, data.Color("Hair", 60))
