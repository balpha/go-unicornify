...
func NewBird(data BirdData) *Bird {
	...
	beakTip := NewBallP(beakOnset.Center.Shifted(Point3d{10, 0, 0}), data.BeakTipSize, data.Color("Beak", 60))
	...
	for _, zs := range [2]float64{-1, 1} {
		...
		wingMiddle := NewBallP(wingAttachBack.Center.Shifted(Point3d{0, 40, zs * 200}), 6, data.Color("Body", 40))
		wingAttachBack.MoveToSphere(*chest)
		wingMiddle.MoveToSphere(*chest)
		wingTip := NewBallP(butt.Center.Shifted(Point3d{0, 0, zs}), 6, data.Color("Head", 50))
